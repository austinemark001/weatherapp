...
function Header(){
  ...
  return(
      ...
      <header className="myheader">
        {location.pathname === '/weather' && <>{location.state && <h1 id='title-weather'>{location.state.current && <sup><img src={`${process.env.PUBLIC_URL}/images/currentlocation.png`} alt='o'/></sup>} 
        {location.state.name ? location.state.name : 'unknown location'}</h1>}</>}
        {location.pathname === '/' &&  <h1 id='title-home'>weather rush <img src={`${process.env.PUBLIC_URL}/images/umbrella1.png`} alt='i'/></h1>}
...
function NoPage(){
  return(
    <div className="noPage">
        <h2 id="nopageInfo">Welcome to weather rush <img src={`${process.env.PUBLIC_URL}/images/umbrella5.png`} alt='i'/></h2>
